...
columnValues <- columnValues[columnValues$idemo == 0, ]

index <- c(1,2)

for (i in columnValues$JLF.Column.Names) {
  tmp <- strsplit(i, split = "%MODALITY%_jlf_%MEASURE%")[[1]][2]
  
  index <- c(index, grep(pattern=tmp, x=names(jlf)))
}



jlf2 <- jlf[, index[order(index)]]

write.csv(jlf2, 
          "/data/joy/BBL/studies/pnc/n2416_dataFreeze/neuroimaging/idemo/n2416_idemo_jlf_roivals_26April2017.csv", 
          row.names=F)
