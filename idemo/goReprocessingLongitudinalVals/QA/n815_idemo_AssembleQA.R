...
##
data1601 <- read.csv("/data/joy/BBL/projects/pncReproc2015/idemo/goReprocessingLongitudinalVals/QA/n815_bblidscanid_list.csv", header=F)
data1601 <- data1601[,1:2]
names(data1601) <-c("bblid","scanid")

#Read in Datasets
dataMeanAct <- read.csv("/data/joy/BBL/projects/pncReproc2015/idemo/goReprocessingLongitudinalVals/QA/n815_idemo_MeanActivation.csv", header=T)
dataMaskCov <- read.csv("/data/joy/BBL/projects/pncReproc2015/idemo/goReprocessingLongitudinalVals/QA/n815_maskCoverage_QA.csv")
dataQA <- read.csv("/data/joy/BBL/projects/pncReproc2015/idemo/goReprocessingLongitudinalVals/QA/n815_idemo_QAMetrics.csv")
...
for (i in 1:dim(dataQA)[1]) {
  temp <- strsplit(dataQA$scanid[i], split = "x")[[1]]
  dataQA$scanid[i] <- temp[length(temp)]
}
...
#Read Max Dataset
maxDat <- read.csv("/data/joy/BBL/projects/pncReproc2015/idemo/goReprocessingLongitudinalVals/QA/n815_idemo_MaxRMS.csv")
...
write.csv(dataQAout, "/data/joy/BBL/projects/pncReproc2015/idemo/goReprocessingLongitudinalVals/QA/n815_idemo_FinalQA.csv", row.names=F)
