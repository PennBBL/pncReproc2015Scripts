...
oldRef <- read.csv('/data/joy/BBL/studies/pnc/n1601_dataFreeze2016/neuroimaging/t1struct/n1601_t1QaData.csv')
...
scanidRedoIndex <- skullIssues$scanid
scanidRedoIndex <- append(scanidRedoIndex, oldRef$scanid[which(oldRef$t1Reprocess==1)])
...
output$t1PostProcessExclude[output$scanid %in% scanidRedoIndex] <- 1
output$t1RawDataExclude[output$scanid %in% scanid0Index] <- 1
output$t1Reprocess[output$scanid %in% scanidRedoIndex] <- 1
...
# Now write the csv 
write.csv(output, '/data/joy/BBL/studies/pnc/n2416_dataFreezeJan2017/neuroimaging/t1struct/n2416_t1QaData.csv', quote=F, row.names=F)
