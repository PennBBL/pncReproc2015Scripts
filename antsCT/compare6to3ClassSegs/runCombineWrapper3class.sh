...
outImg="atropos3class3classmask.nii.gz"
...
parcDir="JLF3classexplore"
scriptToCall=""
templateImg="/data/joy/BBL/studies/pnc/template/pnc_template_brain.nii.gz"
if [ -z ${parcImg} ] ; then 
  echo "${bblid},${scanid},${dateid}" >> ${noParcText}  
else
  /home/arosen/pncReproc2015Scripts/antsCT/compare6to3ClassSegs/antsCTPostProcAndGMD3classMask.sh -d ${antsPath} -o ${outImg} -p ${parcImg} -P ${parcDir} -t ${templateImg} -s 0; 
fi
