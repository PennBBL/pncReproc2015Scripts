...
if [ ! "${antsCheck}" == "/data/joy/BBL/studies" ] ; then
  ...
  echo "/data/joy/BBL/studies/pnc/processedData/structural/antsCorticalThickness/100031/*x3818/"
  exit 5 ; 
fi
...
if [ ${parcCheck} -eq 1 ] ; then
  ...
  parcCommandCT="${AFP}/3dROIstats -nomeanout -1DRformat ${roiValOutput} -nzmean -zerofill NA -mask ${parcMask} ${antsDirectory}CorticalThickness.nii.gz"
  parcCommandGMD="${AFP}/3dROIstats -nomeanout -1DRformat ${roiValOutput} -nzmean -zerofill NA -mask ${parcMask} ${outputImg}_prob02.nii.gz"
  parcCommandGMDIsol="${AFP}/3dROIstats -nomeanout -1DRformat ${roiValOutput} -nzmean -zerofill NA -mask ${parcMask} ${outputImg}_prob02_IsolatedGM.nii.gz"
  ...
  parcCommandGMDVolMod="${AFP}/3dROIstats -1DRformat -nomeanout ${roiValOutput} -nzmean -zerofill NA -mask ${antsDirectory}${parcDir}_subjectToTemplate.nii.gz ${outputImg}_prob02SubjToTempVolumeModulated.nii.gz"
