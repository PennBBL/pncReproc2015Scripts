...
outfiledir="/data/joy/BBL/projects/pncReproc2015/n1601QA/"
...
for i in `tail -n +${startnum} ${inFileAll}` ; do
  ...
  grep ${i} ${inFileBM} 
  if [ $? -eq 0 ] ; then
    flagEcho="${flagEcho} BM"
    bmImage=`ls ${antsCTDir}${bblid}/*${scanid}/BrainExtractionMask.nii.gz` 
    bmFlag=1 ; 
  fi
  grep ${i} ${inFileANTS}
  if [ $? -eq 0 ] ; then
 ...
 fslview ${anatImage} ${gmdImage} ${ctImage} ${gmImage} ${antsImage} ${jlfImage}
  ...
  if [ ${bmFlag} -eq 1 ] ; then
    echo "Enter if the BM values are useable: 1 = Not useable 0 = useable"
    read bmFlag ; 
  fi  
