...
slist=$(cat /data/joy/BBL/studies/pnc/subjectData/go3_subjectlist_20160331_for_FS_process_mq.csv)
...
for i in $slist; do
	...
	subjid=`echo $i | cut -d "/" -f 2`
	echo $subjid
	infile=`ls -d /data/joy/BBL/studies/pnc/rawData/$bblid/$subjid/mprage*/*_t1.nii.gz`
