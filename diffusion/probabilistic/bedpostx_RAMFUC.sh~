...
#for each subject in the list of subjects/timepoints that passed DTI QA (from DRR on dtipipe channel on slack 9/10/15)...
	for i in `cat /data/jag/BBL/studies/pnc/subjectData/go1_bedpostX/go1_dti_qaInclude_RC_batch3.csv`; do
...
#submit the process to the grid, and creates logfiles in the subject's directory. The script bedpostx_run.sh that the qsub command calls merely submits bedpostx by calling "bedpostx". (Could optionally have "bedpostx" here below rather than in a separate script however then logs and other qsub options are not available)

#	qsub -q all.q -V -v bpX_InputPath="${bpX_InputPath}",bblid="${bblid}",datexscanid="${datexscanid}" -m e -M grahamlbaum@gmail.com -wd $logDir /data/jag/BBL/projects/pncReproc2015/pncReproc2015Scripts/diffusion/probabilistic/bedpostx_run.sh
