...
#for each subject in the list of subjects/timepoints that passed DTI QA (from DRR on dtipipe channel on slack 9/10/15)...
	for i in `cat /data/jag/BBL/studies/pnc/subjectData/go1_bedpostX/full_go1_dti_qaInclude_n1210.csv `; do
...
# Source bedpostx directly on chead ** NO QSUB HERE. -c flag is crucial for preventing use of CUDA hardware (which we don't have) **

bedpostx ${bpX_InputPath}/ -c


echo "BedpostX automatically submits bedpost jobs in parallel to all.q for each subject"
echo ""
echo "Logs are automatically generated in $path/$bblid/$datexscanid/bedpostx_input/logs"
