...
# Load data
mdVals <- read.csv('/data/joy/BBL/projects/pncReproc2015/diffusion/prepfaWMLobularVal/merged.csv', header=F)
mdVals <- mdVals[,-15]
...
namesToAdd <- gsub(x=gsub(x=columnNames$JLF.Column.Names, pattern='%MODALITY%', replacement='dti'), pattern='%MEASURE%', replacement='fa')
...
# Now write the csv
write.csv(output.df, paste('/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/dti/n1601_jlfWmLobesFAValues_',format(Sys.Date(), format="%Y%m%d"),'.csv', sep=''), quote=F, row.names=F)
