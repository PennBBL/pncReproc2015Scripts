...
# Load data
mdVals <- read.csv('/data/joy/BBL/projects/pncReproc2015/diffusion/prepadWMLobularVal/merged.csv', header=F)
mdVals <- mdVals[,-15]
...
# Now write the csv
write.csv(output.df, paste('/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/dti/n1601_jlfWmLobesADValues_',format(Sys.Date(), format="%Y%m%d"),'.csv', sep=''), quote=F, row.names=F)
